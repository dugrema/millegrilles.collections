...
import { chiffrage } from '@dugrema/millegrilles.reactjs/src/chiffrage' //'./chiffrage'

import { detecterFormatsVideos } from '@dugrema/millegrilles.reactjs/src/detecterAppareils'
import { blobLoader } from '@dugrema/millegrilles.reactjs/src/imageLoading'
...
function videoLoader(getUrl, creerTokenJwt, videos, opts) {
    ...
    return {
        load: async (optsLoader) => {
            ...
            const { codec: codecOriginal, height, width } = opts
            let original = null;
            if(fuuid && mimetype) {
                original = {fuuid, fuuid_video: fuuid, width, height, codec: codecOriginal, mimetype}
            }

            let videoSelectionne = Object.values(videos).filter(item=>item.fuuid_video === selecteur).pop();
